defmodule OsunsaWeb.AutentificacionController do
  ...
  def delete(conn, _) do
    conn
    |> OsunsaWeb.Guardian.Plug.sign_out()
    |> put_status(:no_content)
    |> json(%{message: "sign out"})
  end

  def refresh(conn, _params) do
    user = OsunsaWeb.Guardian.Plug.current_resource(conn)
    jwt = OsunsaWeb.Guardian.Plug.current_token(conn)

    case OsunsaWeb.Guardian.refresh(jwt, ttl: {1, :days}) do
  ...
  def revoke(conn, params) do
    {:ok, _claims} = OsunsaWeb.Guardian.revoke(params["token"])
